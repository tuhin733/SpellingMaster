...
const FlashcardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !isComplete // Don't start timer if exercise is complete
    ) {
      // Set initial time
      ...
      timerRef.current = setInterval(() => {
        setTimeRemaining((prev) => {
          if (prev === null || prev <= 1) {
            ...
            // Only auto-mark as incorrect if the user hasn't already answered
            if (!results[currentIndex]?.correct) {
              handleResult(false);
            }
  ...
  useEffect(() => {
    ...
    if (isComplete) {
      ...
      const handleResultsAndStreak = async () => {
        // Pass the wordlist ID when updating progress
        ...
        // Update the streak after progress is updated
        await updateStreak(wordlist.id);

        // Check for new achievements
        const updatedStreak = getStreak(wordlist.id);
  ...
  // Add timer display if time limit is active
  const renderTimer = () => {
    if (timeLimit > 0 && timeRemaining !== null) {
      return (
        <div className="flex items-center text-xs sm:text-sm">
          <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1 text-primary-500" />
          <span
      ...
      <main className="flex-1 container-content flex flex-col main-content">
        {isRetrying && (
          <AlertMessage
            type="warning"
            title="Practice Mode"
            message="You're practicing only the words you missed previously. Complete all words correctly to finish this level."
            icon={
        ...
        <Card className="mb-4 sm:mb-6 animate-fade-in dark:bg-secondary-800">
          ...
          <div className="flex justify-between items-center">
            ...
            {/* Timer display - added to the existing UI */}
            {renderTimer()}
