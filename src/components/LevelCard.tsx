...
const LevelCard: React.FC<LevelCardProps> = ({
  ...
  // Use wordlistId to get progress if available, otherwise fallback to language
  const progressKey = wordlistId || language;
  const score = progress?.levelScores?.[level] ?? 0;
  const isCompleted = progress?.completedLevels?.includes(level) || false;
  const incorrectWords = progress?.incorrectWords?.[level] || [];
  ...
  // Show retry label if there are incorrect words
  const showRetry = hasIncorrectWords && !isCompleted;
