...
const UploadModal: React.FC<UploadModalProps> = ({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Save to IndexedDB
      await db.addUserWordlist(wordlist);
            ...
            >
              ...
              {isLoading && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  <div className="w-full bg-secondary-100 rounded-full h-1.5 dark:bg-secondary-700">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${uploadProgress}%` }}
                      className="bg-primary-500 h-1.5 rounded-full dark:bg-primary-400"
                    ></motion.div>
                  </div>
                </motion.div>
              )}
