...
export const AlertMessage: React.FC<{
  ...
  className?: string;
}> = ({ type, title, message, icon, className = "" }) => {
  const typeStyles = {
    info: {
  ...
  return (
    <div
      className={`p-3 sm:p-4 rounded-lg border ${styles.bg} ${styles.border} ${className}`}
    >
      <div className="flex">
        <div className="flex-shrink-0">{icon || styles.icon}</div>
        <div className="ml-2 sm:ml-3">
          <h3 className={`text-xs sm:text-sm font-medium ${styles.text}`}>
            {title}
          </h3>
          <div className={`mt-0.5 sm:mt-1 text-xs sm:text-sm ${styles.text}`}>
            {message}
          </div>
        </div>
      </div>
